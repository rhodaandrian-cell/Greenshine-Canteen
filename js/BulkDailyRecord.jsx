...
function BulkDailyRecord() {
  ...
  async function fetchDupWarning(entry) {
    ...
    try {
      const w = await window.buildDuplicateWarning(
        entry.student.admNo, entry.student.name, date,
        entry.food, entry.tea, entry.porridge
      );
      setDupWarn(w || "");
    } catch (e) { setDupWarn(""); }
  }
  ...
  async function startProcess() {
    const q = pendingList.map(s => {
      ...
      const cost     = (food ? PRICES.FOOD : 0)
                     + (tea  ? PRICES.TEA  : 0)
                     + (porridge ? PRICES.PORRIDGE : 0);
      return { student: s, food, tea, porridge, cost };
    });

    if (!q.length) { bulkToast("No new items ticked", "warning"); return; }
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      setStudents(prev => prev.map(st =>
        st.admNo === s.admNo
          ? { ...st, balance: newBalance, type: res.type || st.type }
          : st
      ));
      setDone(prev => ({ ...prev, [s.admNo]: true }));
      syncWindowStudents(s.admNo, newBalance, res.type);

      // Undo toast
      setLastRecorded({ admNo: s.admNo, prevBalance, name: s.name });
      setUndoVisible(true);
  ...
  function handleUndo() {
    ...
    setStudents(prev => prev.map(st =>
      st.admNo === lastRecorded.admNo ? { ...st, balance: lastRecorded.prevBalance } : st
    ));
    setDone(prev => {
      const n = { ...prev };
    ...
    <>
      ...
      {undoVisible && lastRecorded && (
        <div style={{
          ...
          <span>✅ {lastRecorded.name} recorded</span>
...
function mountBulkRecord() {
  ...
  if (!root) { console.warn("BulkDailyRecord: #bulk-react-root not found"); return; }
